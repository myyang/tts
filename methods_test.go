...
func TestConvertSimple(t *testing.T) {
	url := ConvertSimple("ymy1019ad", "qwer1234", text)
	fmt.Printf("\n\nURL: %#s\n\n", url)
	if url == "" {
...
func TestConvertText(t *testing.T) {
	url := ConvertText("ymy1019ad", "qwer1234", text, "MCHEN_Joddess", "80", "3", "wav")
	fmt.Printf("\n\nURL: %#s\n\n", url)
	if url == "" {
		t.Fatalf("Fail to get file")
	}
...
func TestConvertAdvancedText(t *testing.T) {
	url := ConvertAdvancedText("ymy1019ad", "qwer1234", text, "Angela", "95", "2", "wav", "1", "1", "10")
	fmt.Printf("\n\nURL: %#s\n\n", url)
	if url == "" {
		t.Fatalf("Fail to get file")
	}
